...
snapshot=2
show_planes = [["XY",2,mid_z,snapshot,0],["YZ",2,mid_x,snapshot,1],["YZ",2,mid_x,snapshot,0]]
...
def Grilla_XZ():
        ...
        R = 1
        P,T  = np.meshgrid(domain_x,domain_z) #Entrega radio y co-latitud
        X,Z = R*np.cos(P),R*np.cos(T)
        return X,Z
...
def plot_YZ(type_map,cut_x,snapshot,unidad,azim=False):
        global cmap                                                                                                                     #globlaliza la variable cmap
        if type_map == 2:                                                                                                       #Condicion para obtener Temperatura, (depende de dos mapas densidad y energia)
        ...
        if not azim:return Y,Z,data
        else:return X,Z,data
...
for plot in show_planes:
        ...
        #Plano YZ ---------------------------------------------------------#
                ...
                #Plot principal -----------------------------------------------#
                Y,Z,data = plot_YZ(plot[1],plot[2],plot[3],plot[1],azim)
                if limits == True:
